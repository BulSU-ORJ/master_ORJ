...
.media-body #collapseExample.collapse:not(.show) {
  display: block;
  height: 3rem;
  overflow: hidden;
}

.media-body #collapseExample.collapsing {
  height: 3rem;
}

.media-body a.collapsed::after {
  content: 'Read More';
    font-size: 13px;
}

.media-body a:not(.collapsed)::after {
    content: 'Show Less';
    font-size: 13px;
}
</style>
    ...
    <?php 
      ...
      <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent" >
            ...
            <a href="B-CollegesRegistered.php" style="text-decoration:none; font-weight: bold" id="disabled">COLLEGES</a>
      ...
      <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
        ...
        <div class="collapse navbar-collapse" data-hover="dropdown" data-animations="fadeInDown" id="navbarSupportedContent" >
            ...
            <li style="list-style-type:none;"><a class="btn nav-item" href="B-Colleges.php" style="font-weight: bold" id="disabled">COLLEGES</a></li>
...
<div class="container mb-5" id="comments"><hr>
                ...
                while($row=$result->fetch_array()){
                    if(password_verify($row['researchNo'],$_SESSION["random_id"])){
                        ...
                        <div class="thumbnail rounded">
                            ...
                            if($result2){
                                while($row2=$result2->fetch_array()){
                                    if(($_SESSION['username']==$row2['likedBy']) && ($row['name']==$row2['username']) && ($row['researchNo']==$row2['researchNo'])){
                                        $reacts='';
                                        $attr2='disabled';
                                        $reacted='Liked';
                                    }
                                ...
                                $comments.='
                                        <button type="button" class="btn btn-dark text-center col-sm-2" id="'.$row['name'].'" onclick="toLikes(this.id)" '.$attr2.'><i class="fa fa-heart pr-2"> '.$reacts.' '.$reacted.'</i></button></div></div></div></div>';
                                $reacts=0;
                                $reacted='';
                            }else{
                        ...
                        <div class="thumbnail rounded">
                            ...
                            if($result2){
                                while($row2=$result2->fetch_array()){
                                    if(($row['name']==$row2['username']) && ($row['researchNo']==$row2['researchNo'])){
                                        $reacts+=1;
                                    }
                                }
                                $comments.='
        ...
        ?>
    <h4 class="media-header"><?php echo $count.' Comments'; ?></h4>
		 <?php echo $comments; ?>
</div>
<!--scrollspy-->
<script>
