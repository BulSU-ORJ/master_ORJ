<?php
    ...
    if($_SESSION['flag']==1){
        $query='SELECT * FROM `uploaddata` order by `author_category` ASC';
        $stmt = $db->query($query);
	   $count=1;
	   ...
	   while($data=$stmt->fetch(PDO::FETCH_OBJ)){
        ...
        $query='SELECT * FROM `uploaddata` WHERE `author_category`="'.$_SESSION['category'].'" order by `author_category` ASC';
        $stmt = $db->query($query);
        $count=1;
        $pdf->SetTextColor(0,0,0);
        if($stmt->rowCount()>0){
