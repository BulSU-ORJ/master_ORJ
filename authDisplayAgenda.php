...
class authDisplay{
    function agendaDisplay($agenda,$tableNo) {
		$db_handle = new DBController();
	    $query = "Select * from `uploaddata` where `agenda` = ? ORDER BY ID DESC";
	    $result = $db_handle->runQuery($query, 's', array($agenda));
		if(!empty($result)){
		...
		foreach($result as $row){
			...
			echo    '<td class="text-center" id="'.$hash.'" onclick="clickTable(this.id)">'.date('F',strtotime($row['date'])).' '.date('d',strtotime($row['date'])).', '.date('Y',strtotime($row['date'])).'</td>';
